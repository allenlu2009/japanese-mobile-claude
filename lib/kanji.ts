 ...
 *
 * Complete JLPT-based kanji organized by difficulty level (N5, N4, and N3).
 * Data is loaded from JSON files for easier maintenance and expansion.
 *
 * Data source: JLPTsensei.com + open sources (January 2026)
 * N5: 80 kanji
 ...
 */
export function getKanjiByLevel(level: 'N5' | 'N4' | 'N3', includeLower: boolean = true): KanjiChar[] {
  if (level === 'N5') {
    return ALL_KANJI_N5;
  } else if (level === 'N4') {
 ...
 * @param includeLower - Include lower levels: true (default) or false (level-only)
 */
export function getKanjiCount(level: 'N5' | 'N4' | 'N3', includeLower: boolean = true): number {
