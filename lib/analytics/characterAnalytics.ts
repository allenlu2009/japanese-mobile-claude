import type { CharacterAttempt } from '../types';

export interface CharacterStats {
  character: string;
  scriptType: 'hiragana' | 'katakana' | 'kanji' | 'vocabulary';
  characterType: 'basic' | 'dakuten' | 'combo' | null;
...
export function calculateCharacterStats(
  ...
  // Get character type and script type from first attempt
  const scriptType = characterAttempts[0].scriptType;
  const characterType = characterAttempts[0].characterType;
  const jlptLevel = characterAttempts[0].jlptLevel;
  const readingType = characterAttempts[0].readingType;
  ...
  const allTimeSuccessRate = successRate;
  ...
  // Get date range
  const timestamps = characterAttempts.map(a => a.timestamp);
...
/**
 * Get kana stats only (exclude kanji and vocabulary)
 */
export function getKanaStats(stats: CharacterStats[]): CharacterStats[] {
  return stats.filter(s => s.scriptType === 'hiragana' || s.scriptType === 'katakana');
}

/**
 * Get kanji/vocab stats only
 */
export function getJLPTStats(stats: CharacterStats[]): CharacterStats[] {
  return stats.filter(s => s.scriptType === 'kanji' || s.scriptType === 'vocabulary');
}

/**
 * Group stats by JLPT level
 */
...
/**
 * Get stats for a specific script type
 */
export function getStatsByScriptType(
  stats: CharacterStats[],
  scriptType: 'hiragana' | 'katakana' | 'kanji' | 'vocabulary'
): CharacterStats[] {
  return stats.filter(s => s.scriptType === scriptType);
}
